...
> There are other options for the `ON DELETE` setting, but we will not use them in this tutorial.
...
> You will learn about different types of joins later.
...
Let's create an order for the user with the id `1`.
...
> We also added `ON DELETE RESTRICT` setting to **reject the command to delete the product** if there are created orders
> with it.
...
Now, having the orderd_id you can get all the products for that order by adding `WHERE` clause:
